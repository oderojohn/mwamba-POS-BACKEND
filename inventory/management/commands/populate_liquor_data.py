...
class Command(BaseCommand):
    help = 'Populate database with sample liquor products'

    def handle(self, *args, **options):
        self.stdout.write('Clearing existing data...')
        # Clear existing data first
        from inventory.models import Product, Category, Batch, StockMovement, SalesHistory
        ...
        self.stdout.write('Creating sample liquor data...')
        ...
        suppliers_data = [
            {
                ...
                'phone': '+254700123456',
                'email': 'john@kenyawine.co.ke',
                'address': 'Nairobi Industrial Area, Kenya'
            },
            {
                'name': 'East African Spirits Ltd',
                'contact_person': 'Sarah Oduya',
                'phone': '+254711234567',
                'email': 'sarah@easpirit.co.ke',
                'address': 'Mombasa Road, Nairobi, Kenya'
            },
            {
                'name': 'Premium Imports Kenya',
                'contact_person': 'David Kiprop',
                'phone': '+254722345678',
                'email': 'david@premiumimports.co.ke',
        ...
        # Create products with retail and wholesale pricing
        products_data = []
        ...
        for prod_data in products_data:
            ...
            if created:
                self.stdout.write(f'Created product: {product.name} (Stock: {product.stock_quantity})')

        # Create sample customers
        customers_data = [
            {
                'name': 'Koinange Street Wines & Spirits',
                'phone': '+254700111222',
                'email': 'orders@koinangestreet.co.ke',
                'customer_type': 'wholesale',
                'credit_limit': Decimal('50000.00'),
                'credit_balance': Decimal('0.00'),
                'business_name': 'Koinange Street Wines & Spirits Ltd'
            },
            {
                'name': 'Westlands Liquor Store',
                'phone': '+254711333444',
                'email': 'manager@westlandsliquor.co.ke',
                'customer_type': 'wholesale',
                'credit_limit': Decimal('75000.00'),
                'credit_balance': Decimal('0.00'),
                'business_name': 'Westlands Liquor Store Ltd'
            },
            {
                'name': 'James Mwangi',
                'phone': '+254722555666',
        ...
        self.stdout.write(self.style.SUCCESS('Sample liquor data populated successfully!'))
        self.stdout.write(f'Created {len(categories)} categories, {len(products)} products, {len(suppliers)} suppliers, and {len(customers_data)} customers')